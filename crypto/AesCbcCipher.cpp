...
bool AesCbcCipher::encrypt(const Vuc& in, Vuc& out)
{
    ...
    // init encryption
    EVP_CIPHER_CTX ctx;
    EVP_EncryptInit(&ctx, getCipher(), &key_[0], &iv_[0]);

    // do the encrypt; must keep track of the number of bytes produced
    int nBytes = 0;
    EVP_EncryptUpdate(&ctx, &out[0], &nBytes, &in[0], in.size());
    int nTotalBytes = nBytes;
    EVP_EncryptFinal(&ctx, &out[nBytes], &nBytes);
    nTotalBytes += nBytes;
...
bool AesCbcCipher::decrypt(const Vuc& in, Vuc& out)
{
    ...
    EVP_CIPHER_CTX ctx;
    EVP_DecryptInit(&ctx, getCipher(), &key_[0], &iv_[0]);

    // do the decrypt
    int nBytes = 0;
    EVP_DecryptUpdate(&ctx, &out[0], &nBytes, &in[0], in.size());
    int nTotalBytes = nBytes;
    if (!EVP_DecryptFinal(&ctx, &out[nBytes], &nBytes))
